<div class="max-w-7xl mx-auto px-4 py-2">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        @if(isset($newArrivals) && $newArrivals->count() > 0)
                ...
                <div class="p-4 group relative bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300">
                    ...
                    <div class="w-full aspect-square relative">
                        ...
                        <div class="w-full h-full rounded-lg overflow-hidden">
                            ...
                            <img src="{{ $product->primaryImage ? asset($product->primaryImage->image_path) : 'https://placehold.co/400x400' }}" 
                    ...
                    <div class="mt-2 text-center">
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-center gap-2">
                                <span class="text-orange-500 font-semibold">${{ number_format($product->displayPrice, 2) }}</span>
                                @if($product->displayPrice < $product->base_price)
                                <span class="text-gray-400 text-sm line-through">${{ number_format($product->base_price, 2) }}</span>
                                @endif
                            </div>
                            <a href="{{ route('product.details', ['id' => $product->id, 'slug' => $product->slug]) }}" class="w-full flex items-center justify-center space-x-2 bg-orange-500 text-white px-4 py-2 rounded-full text-sm hover:bg-orange-600 transition-colors">
            ...
            @endforeach
        @else
            <!-- Fallback display if no products are available -->
            <div class="col-span-full text-center py-8">
                <p class="text-gray-500 text-lg">No new products available at the moment.</p>
                <p class="mt-2">Check back soon for our latest arrivals!</p>
            </div>
        @endif
    </div>
</div>